...
# converting training lables to 2 class intead of 10
 ...
 # converting test lables to 2 class intead of 10 
  ...
  W = tf.Variable(tf.random_normal([784 , 2] , name="weights"))
  b = tf.Variable(tf.zeros([2]))#2
  y = tf.matmul(x, W) + b

  #comment these for subdivision 1
 ...
 #loss function
  cross_entropy = tf.reduce_mean(
      tf.nn.softmax_cross_entropy_with_logits(labels=y_, logits=y)) # change to y for subdivision 1
  ...
  # Test trained model
  correct_prediction = tf.equal(tf.argmax(y, 1), tf.argmax(y_, 1)) # for first subdivision change to y
  ...
  sess_pred = sess.run(correct_prediction,feed_dict={x: mnist.test.images,
                                      y_: mnist_test_labels})
  # miscalssification finding and visualization
  print(sess_pred)
  ...
  for i in mis_classify:
  	if(mnist_test_labels[i][1] == 1):
  		if(odd < 10):
  			image = mnist.test.images[i]
  			image_reshape = image.reshape((28,28))
  			plt.figure(1)
  			plt.subplot2grid((2,5),(odd //5 , odd % 5))
  			plt.imshow(image_reshape,cmap = "gray")
  			odd = odd + 1
  	else:
  		if(even < 10):
  			image1 = mnist.test.images[i]
  			image1_reshape = image1.reshape((28,28))
  			plt.figure(2)
  			plt.subplot2grid((2,5),(even //5 , even % 5))
  			plt.imshow(image1_reshape,cmap="gray")
  			even = even + 1
  	if(even + odd == 20):
  		fig1 = plt.figure(1)
  		fig1.suptitle("Odd samples which are misclassified")
  		fig2 = plt.figure(2)
  		fig2.suptitle("Even samples which are misclassified")
  		break
  plt.show()
